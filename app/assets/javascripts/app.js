...
meuApp.controller('principal', function($scope, $resource, $http){
    ...
    var pedidoJson=$resource("/api/colaboradors/:id", null, {
   ...
   $scope.zeraInsumoAll= function(){
    ...
    $scope.somaCaixa= function(){
        ...
        pedidoJson.save({ pedido: $scope.pedidoDados, function() {
            ...
            $scope.valorTotal=0;
             ...
             pedidoJson.query(function(data){
                ...
                //for que que soma os valores dos itens
                      for(var i=0 ; i < $scope.estoqueProd.length ; i++){
                         $scope.valorTotal += parseFloat($scope.estoqueProd[i].valor * $scope.estoqueProd[i].qtd);
                          $scope.subtraiQtd($scope.estoqueProd[i].estoque_id,$scope.estoqueProd[i].qtd);
                           
                      }
                            console.log("testeeee= "+$scope.valorTotal);

                         //salva os dados no itens caixa
                        $scope.cadastraItensCaixa($scope.valorTotal,formapgto);
     ...
     $scope.teste= function(){
      alert("foi");
              $scope.cadastraItensCaixa();
                 
     }
     
     // subtrai a quantidade do iten no estoque
     $scope.subtraiQtd= function(id,qtd){

                estoqueJson.get({id:id}, function(data){
                   
                   var qtdAtual = data.quantidade;
                   var qtdItens= qtd;
                   var total= qtdAtual - qtdItens;
                   data.quantidade= total;
                   alert("id="+id+" qtdAtual = "+ qtdAtual + "qtdItens" + qtdItens +"total");
                   data.$update();
                   
                });
     }

     $scope.cadastraItensCaixa= function(valorTotal,formapgto){

                  caixaJson.query(function(data){
                     var lastid= data.length - 1;
                     var lastCaixa=data[lastid].id;
                     
                    
                     //salva os dados no itens caixa
                         $scope.dados={total: valorTotal, payment_id:formapgto, caixa_id:lastCaixa};
