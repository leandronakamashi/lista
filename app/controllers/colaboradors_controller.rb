class ColaboradorsController < ApplicationController
  ...
  def index
    ...
    @q = Colaborador.ransack(params[:q])
    @colaboradors=@q.result
  end

def ok
    respond_to do |format|
      if @colaborador.update({checkin: "Sim"})
        format.html { redirect_to "/checkins" , notice: 'Check in realizado com sucesso.' }
  ...
  def update
    respond_to do |format|
      if @colaborador.update(colaborador_params)
        format.html { redirect_to "/colaboradors" , notice: 'Colaborador was successfully updated.' }
        format.json { render :show, status: :ok, location: @colaborador }
      else
        format.html { render :edit }
        format.json { render json: @colaborador.errors, status: :unprocessable_entity }
  ...
  private
    ...
    def colaborador_params
      params.require(:colaborador).permit(:nome, :hotel, :setor, :transporte, :data_envio, :data_confirma, :chave_entrega, :preimo, :checkin)
    end
end
